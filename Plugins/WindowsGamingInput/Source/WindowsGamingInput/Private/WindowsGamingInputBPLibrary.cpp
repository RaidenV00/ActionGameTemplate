...
const float c_XboxOneThumbDeadZone = .0f;  // Recommended Xbox One controller deadzone
...
void ScanGamePads()
{
	...
	for (size_t j = 0; j < MAX_PLAYER_COUNT; ++j)
	{
		if (gamepads[j])
		{
			...
			{
				ComPtr<IGamepad> pad;
				HRESULT thr = pads->GetAt(k, pad.GetAddressOf());
				if (SUCCEEDED(thr) && (pad == gamepads[j]))
				{
					break;
				}
			}
			...
			if (k >= count)
			{
				...
				HRESULT thr = gamepads[j].As(&ctrl);
				if (SUCCEEDED(thr) && ctrl)
				{
...
INIT_ONCE tg_InitOnce = INIT_ONCE_STATIC_INIT; // Static initialization
...
HANDLE InitializeGamepad()
{
	...
	bStatus = InitOnceExecuteOnce(&tg_InitOnce,          // One-time initialization structure
